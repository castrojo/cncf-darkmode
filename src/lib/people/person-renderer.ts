// person-renderer.ts — shared card renderer for PersonCard.astro and feed-loader.ts.
...
export interface PersonEvent { id: string; type: string; timestamp: string; person: Person; changes?: Change[]; }
...
export const LOGO_PRIORITY = [
  'Golden-Kubestronaut', 'Kubestronaut', 'Ambassadors',
  'Technical Oversight Committee', 'End User TAB', 'Staff',
];
...
// Canonical type labels — 'removed' = 'Emeritus' (CNCF term for alumni).
...
export function renderPersonCard(e: PersonEvent, landscapeLogos: Record<string, string>): string {
  ...
  const statsRow = (p.contributions || p.publicRepos || p.yearsContributing) ? `<div class="stats-row">${
    p.yearsContributing ? `<span class="stat-chip"><img src="${BASE}/program-logos/cncf.svg" alt="" class="stat-cncf-icon" aria-hidden="true" style="width:14px;height:14px;object-fit:contain;flex-shrink:0"><span class="stat-val">Since ${currentYear - p.yearsContributing} (${p.yearsContributing}y)</span></span>` : ''}${
  ...
  const changesHtml = e.changes?.length ? `<details class="changes-details"><summary class="changes-summary">${e.changes.length} field${e.changes.length > 1 ? 's' : ''} changed</summary><ul class="changes-list">${
    e.changes.map(c => `<li class="change-item"><span class="change-field">${esc(c.field)}</span><span class="change-from">${esc(c.from || '(empty)')}</span><span class="change-arrow">→</span><span class="change-to">${esc(c.to || '(empty)')}</span></li>`).join('')
  }</ul></details>` : '';

  const rightCol = (programLogo || p.location) ? `<div class="card-right">${
  ...
  const socialLinks =
    ...
    sl(p.github, 'GitHub', GH_ICON) +
    sl(p.linkedin, 'LinkedIn', LI_ICON) +
    sl(p.twitter, 'Twitter/X', TW_ICON) +
    sl(p.bluesky, 'Bluesky', BSKY_ICON);
  ...
  return `<article class="person-card" data-id="${esc(e.id)}" data-type="${esc(e.type)}" data-category="${esc(primaryCat.toLowerCase())}" data-categories="${esc(cats.map(c => c.toLowerCase()).join('|'))}" style="--card-accent:${catInfo.color}">` +
    ...
    `<div class="badges"><span class="badge badge-${esc(e.type)}">${esc(typeLabel)}</span>${catBadges}</div>` +
    `</div></div>` +
